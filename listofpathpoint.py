...
def package_points(matrices):
    ...
    for matrix in matrices:
        ...
        if (int(long_side/6) % 2 == 0):
            corners = [((x,y),(x + len(rectangle),y)),((x,y + len(rectangle[0])),(x + len(rectangle),y + len(rectangle[0])))]
        else :                       
            corners = [(((x,y),(x + len(rectangle),y + len(rectangle[0])))),((x+ len(rectangle),y) ,(x,y + len(rectangle[0])))]
